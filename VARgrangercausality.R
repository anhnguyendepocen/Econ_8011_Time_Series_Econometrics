...
# no idea what this is really doing!
causality(var.2c, cause = "fire")
...
# Each variable Granger cause fire?
ur <- lm(lagf[,1]~lagf[,2:4]+lags[,2:4]+lagt[,2:4])
summary(ur)
rr <- lm(lagf[,1]~lagf[,2:4]+lags[,2:4])
anova(rr,ur,test="F")
rr <- lm(lagf[,1]~lagf[,2:4]+lagt[,2:4])
anova(rr,ur,test="F")
rr <- lm(lagf[,1]~lags[,2:4]+lagt[,2:4])
anova(rr,ur,test="F")

# Each variable Granger cause serv?
ur <- lm(lags[,1]~lagf[,2:4]+lags[,2:4]+lagt[,2:4])
summary(ur)
rr <- lm(lags[,1]~lagf[,2:4]+lags[,2:4])
anova(rr,ur,test="F")
rr <- lm(lags[,1]~lagf[,2:4]+lagt[,2:4])
anova(rr,ur,test="F")
rr <- lm(lags[,1]~lags[,2:4]+lagt[,2:4])
anova(rr,ur,test="F")
